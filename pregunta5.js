...
function arreglarNombre(beer) {
  var nombre = beer.name;
  var extension = ".png";
...
function actualizarSitio(beer) {
  var aux = beer.label.split("/");
  beer.label =
    "https://tecnoshare.sharepoint.com/sites/beer/" +
    aux[5] +
    "/" +
    arreglarNombre(beer);
}

beers.map((beer) => actualizarSitio(beer));
